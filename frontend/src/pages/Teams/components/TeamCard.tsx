import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import { ArrowRight, Users } from 'lucide-react';
import type { PublicTeam } from '@types';
import { TeamCrest } from '@/components/public';

export default function TeamCard({ team }: { team: PublicTeam }) {
	const { t } = useTranslation();
	const color = team.primaryColor || '#0F172A';

	return (
		<Link
			to={`/teams/${team.id}`}
			className="group relative flex flex-col gap-4 overflow-hidden rounded-2xl border border-border bg-card p-5 pl-6 shadow-sm transition-shadow hover:shadow-md"
		>
			{/* The team's own colour is the card's only strong mark. */}
			<span aria-hidden className="absolute inset-y-0 left-0 w-1" style={{ backgroundColor: color }} />

			<div className="flex items-center gap-3.5">
				<TeamCrest team={team} size={44} />
				<div className="flex min-w-0 flex-col gap-0.5">
					<h2 className="truncate text-lg font-bold leading-tight">{team.name}</h2>
					<p className="truncate text-[13px] text-subtle-foreground">
						{team.season?.league?.name ?? t('public.teams.noLeague')}
					</p>
				</div>
			</div>

			<div className="flex items-center gap-5 border-t border-border-subtle pt-3.5 text-[13px] text-subtle-foreground">
				<span className="flex items-center gap-2">
					<Users className="size-4 text-muted-foreground" />
					{t('public.players.count', { count: team._count?.players ?? 0 })}
				</span>
				<span className="truncate">
					{team.season ? team.season.name : t('public.teams.noSeason')}
				</span>
				<span className="ml-auto flex items-center gap-1.5 font-semibold text-primary">
					{t('public.teams.view')}
					<ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" />
				</span>
			</div>
		</Link>
	);
}
